...
from app.models.schemas.attorney_account import  RegisterAttorneyAccountModel
...
router = APIRouter(
    prefix="/attorney/register",
    tags=["Attorney"]
)

@router.post("/")
async def register_attorney(request: Request, 
    ...
    lastId = db.query(DB_Attorney_Users).order_by(DB_Attorney_Users.id.desc()).first().id + 1
    
    payload = RegisterAttorneyAccountModel(id=lastId,
    ...
    obj = DB_Attorney_Users(**payload.dict())
    db.add(obj)
    db.commit()
    db.refresh(obj)
...
def validate_existing_user(email, mobile_number, db):
    if db.query(DB_Attorney_Users).filter(DB_Attorney_Users.email == email).first():
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="User with this email already exists")
    if db.query(DB_Attorney_Users).filter(DB_Attorney_Users.mobile_number == mobile_number).first():
