...
async def get_account(id :int , request: Request, db: Session = Depends(get_db), get_current_user: int = Depends(get_current_user)):
    ...
    query = db.query(DB_Mentor_Users.id, DB_Mentor_Users.suffixe_name, 
                    ...
                    DB_Mentor_Users.working_hours_monday, DB_Mentor_Users.working_hours_tuesday, DB_Mentor_Users.experience_since,
...
async def get_mentorAvaliableWithin60min(catId :int, after: int, request: Request, db: Session = Depends(get_db), get_current_user: int = Depends(get_current_user)):
    myHeader = validateLanguageHeader(request)
    ...
    booking_time = (datetime.now() + timedelta(hours=after))
    ...
    query_of_mentors_depend_on_category =  db.query(DB_Mentor_Users, DB_Mentor_Users.id, DB_Mentor_Users.gender, DB_Mentor_Users.suffixe_name,
                                                    ...
                                                    DB_Mentor_Users.date_of_birth, DB_Mentor_Users.experience_since,
                                                    ...
                                                    DB_Mentor_Users.working_hours_thursday, DB_Mentor_Users.working_hours_friday,
                                                    DB_Mentor_Users.speaking_language).filter(DB_Mentor_Users.category_id == catId
                                                                            ).filter(DB_Mentor_Users.blocked == False).all()
    
    list_of_mentors: list[MentorFilter1] = []
    for mentor in query_of_mentors_depend_on_category:
        if speaking_language in mentor["speaking_language"]:
            obj = MentorFilter1(id = mentor["id"], gender =  mentor["gender"], suffixe_name = mentor["suffixe_name"], 
                                                             first_name = mentor["first_name"], last_name = mentor["last_name"], 
                                                             profile_img = mentor["profile_img"], hour_rate = mentor["hour_rate"],
                                                             bio = mentor["bio"],  date_of_birth = mentor["date_of_birth"], 
                                                             working_hours = [])
    ...
    for mentor in list_of_mentors:
        ...
        list_of_mentors[index].rate = getAverage(list_of_stars)
        index = index + 1
