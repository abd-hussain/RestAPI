...
async def get_Posts(request: Request, db: Session = Depends(get_db), get_current_user: int = Depends(get_current_user), limit: int = 10, skip: int = 0, search: Optional[str] = ""):
    ...
    data = db.query(DB_Notifications.id, DB_Notifications.title_english.label(
        ...
        "content"), DB_Notifications.readed).filter(DB_Notifications.client_owner_id == get_current_user.user_id).filter(DB_Notifications.title_english.contains(search)).limit(limit).offset(skip).all()
    if (myHeader.language == "ar"):
        data = db.query(DB_Notifications.id, DB_Notifications.title_arabic.label(
            ...
            "content"), DB_Notifications.readed).filter(DB_Notifications.client_owner_id == get_current_user.user_id).filter(DB_Notifications.title_arabic.contains(search)).limit(limit).offset(skip).all()
...
@router.put("/client")
async def mark_As_reded_Notification(id :int ,status :bool ,request: Request, db: Session = Depends(get_db), get_current_user: int = Depends(get_current_user)):
    myHeader = validateLanguageHeader(request)
    ...
    if status != None:
        query.update({"readed" : status}, synchronize_session=False)
