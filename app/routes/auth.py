...
from app.models.schemas.account_model import AccountModel, AccountVerifyModel
...
def authentication(payload: AccountModel, db: Session =  Depends(get_db)):
    post_query = db.query(db_user.DB_Users).filter(db_user.DB_Users.mobile_number == payload.mobile_number)
    ...
    post_query.update({"last_otp" : payload.last_otp}, synchronize_session=False)
    db.commit()
    return general.generalResponse(message= "OTP Sended successfully", data=post_query.first())
...
def verify_otp(payload: AccountVerifyModel, db: Session =  Depends(get_db)):
    ...
    if not verifyKey(payload.otp, post_query.first().last_otp):
